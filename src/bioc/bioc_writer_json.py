...
class BioCJsonReader:
    ...
    def _write_collection(self, directory):
        documents = []
        for d in tqdm(self.collection.documents):
            fn = directory + '/' + d.id
            d.id = d['id']
            d.infons = d['infons']
            d.passages = self._read_passages(d['passages'])
            d.relations = self._read_relations(d['relations'])
            d.append(document)
